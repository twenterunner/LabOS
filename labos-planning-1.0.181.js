...
class PlanningEngine{
  ...
  optimizeRecovery(id){const e=this.entity(id);if(!e)throw new Error('Programme was not found.');const currentForecast=isoDay(e.forecastDate),required=isoDay(e.requiredDate),comparison=this.compareLabs(id);const rows=comparison.rows.filter(x=>x.ok).map(x=>{const improvementDays=currentForecast&&x.forecastDate?daysBetween(x.forecastDate,currentForecast):0,baselineLate=required&&currentForecast?Math.max(0,daysBetween(required,currentForecast)):null,newLate=required&&x.forecastDate?Math.max(0,daysBetween(required,x.forecastDate)):null,improves=currentForecast?((newLate??0)<(baselineLate??0)||String(x.forecastDate)<String(currentForecast)):true;return {...x,currentForecast,improvementDays:Math.max(0,improvementDays),baselineLateDays:baselineLate,newLateDays:newLate,improves}}).sort((a,b)=>(b.improves-a.improves)||(a.newLateDays??9999)-(b.newLateDays??9999)||String(a.forecastDate||'9999').localeCompare(String(b.forecastDate||'9999')));return {...comparison,currentForecast,requiredDate:required||null,rows,improvements:rows.filter(x=>x.improves)}}
