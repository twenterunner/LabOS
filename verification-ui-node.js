...
checks.push(['visible revision badge is populated',els['#versionBadge'].textContent==='REV 1.0.7']);
...
checks.push(['AUTO-PLAN requires material and process/test inputs first',appSource.includes('A schedule is not credible yet')&&appSource.includes('Material feasibility first')&&appSource.includes('Process and test methods must be known')]);
checks.push(['standard tests and lab skills are exposed as planning master data',appSource.includes('STANDARD TEST LIBRARY')&&appSource.includes('SKILL CATALOGUE')&&appSource.includes('Training certificates & staff coverage')]);
...
checks.push(['digital traveller records start/complete actual duration',appSource.includes('Start controlled step')&&appSource.includes('actualDurationHours')&&appSource.includes('Actual wall-clock duration is retained')]);
