...
class MigrationService{static migrate(state){let s=P.deepClone(state);if(s.schemaVersion>P.SCHEMA_VERSION)throw new Error('This data was created by a newer schema.');while(s.schemaVersion<P.SCHEMA_VERSION){if(s.schemaVersion===0){s.settings=s.settings||{};s.schemaVersion=1;}else throw new Error(`No migration available from schema ${s.schemaVersion}`);}return s;}}
