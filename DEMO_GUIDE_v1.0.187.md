# LabOS REV 1.0.187 — Management Demo Guide

## REV 1.0.187 planning recovery

Planning is treated as a release-critical function in this revision. Overall, per-programme and per-equipment views continue to use the same constrained planning model. REV 1.0.187 specifically hardens Overall swim-lane interaction, target-lab resource scoping, manual replan access and the formal sister-lab request/approval handshake.

For the demo, use Overall Planning first: verify the six headline tiles stay on one row, switch All / Prototype Builds / Validation, drag the timeline horizontally, click a booking for Green / Yellow / Red manual alternatives, run Optimize recovery for local-only recovery, and use Compare sister labs to create a request. Then switch to the receiving laboratory and accept/revalidate the request before the LIVE plan moves.
...
Open **Help → Management Demo**. REV 1.0.187 contains 16 prepared showcase records. The strongest new end-to-end example is **V26-0205**.
...
## Planning interaction additions in REV 1.0.187
