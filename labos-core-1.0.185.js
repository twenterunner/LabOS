(function(){
  ...
  ProtoLab.materialPlanningAssessment = (state,r) => {
    ...
    const reqs=r.materialRequirements||[],alloc=(state.allocations||[]).filter(a=>a.requestId===r.id),today=ProtoLab.todayISO(),outputLimit=ProtoLab.materialOutputLimit(state,r),requirementsValid=reqs.length>0&&reqs.every(req=>Number(req.requiredQty)>0);
    const issuedReady=requirementsValid&&reqs.every(req=>alloc.filter(a=>a.requirementId===req.id&&a.status==='Issued').reduce((n,a)=>n+Number(a.qty||0),0)>=Number(req.requiredQty||0));
    if(r.materialOwnership==='Engineering supplied'){
      const supply=r.materialSupply||{}; const planningReady=issuedReady||(requirementsValid&&!!(supply.expectedDate&&supply.owner)),commitmentReady=issuedReady||(planningReady&&(supply.committed===true||supply.commitmentStatus==='Committed')),partial=outputLimit.maxBuildQty>0&&!issuedReady;
      return {source:r.materialOwnership,planningReady,commitmentReady,workflowReady:commitmentReady,buildReady:issuedReady,earliestDate:issuedReady?(supply.receivedAt||today):(supply.expectedDate||null),owner:supply.owner||r.requester,summary:issuedReady?'Exact engineering-supplied BOM material is received and issued':partial?`Partial material is issued: current output limit ${outputLimit.maxBuildQty}/${outputLimit.requestedQty} units`:commitmentReady?`Engineering committed the complete BOM quantity for ${supply.expectedDate}`:planningReady?`Engineering supply timing is defined for ${supply.expectedDate}, but the complete BOM quantity is not yet formally committed`:'Engineering supply date/owner not yet defined',issues:issuedReady?[]:partial?[`Current issued material limits output to ${outputLimit.maxBuildQty}/${outputLimit.requestedQty} requested unit(s). Receive the remaining BOM quantity to remove the limiter.`]:commitmentReady?[]:planningReady?['Confirm that the complete controlled BOM quantities are committed to this build before closing the Material workflow step.']:['Record who supplies the BOM material and its expected lab arrival date, then confirm the complete BOM quantity commitment or receive the material directly.'],outputLimit};
    }
    ...
    const reservedReady=requirementsValid&&lines.every(x=>x.reserved>=Number(x.req.requiredQty||0)),supplyPlanReady=requirementsValid&&lines.every(x=>x.covered),partial=outputLimit.maxBuildQty>0&&!issuedReady,arrivalDates=lines.filter(x=>x.reserved<Number(x.req.requiredQty||0)&&x.expectedDate).map(x=>x.expectedDate),earliestDate=arrivalDates.length?arrivalDates.sort().at(-1):today;
    return {source:'Lab supplied',planningReady:supplyPlanReady,commitmentReady:supplyPlanReady,workflowReady:supplyPlanReady,buildReady:issuedReady,earliestDate,owner:(state.users||[]).find(u=>u.role==='lab_planner')?.name||'Lab Planner',summary:issuedReady?'Exact BOM material is issued for the full build':partial?`Partial BOM material is issued: current output limit ${outputLimit.maxBuildQty}/${outputLimit.requestedQty} units`:reservedReady?'Exact BOM material is reserved from lab stock':supplyPlanReady?`Reserved plus committed incoming supply covers every exact BOM quantity; latest expected arrival ${earliestDate}`:lines.every(x=>x.ok)?'Exact BOM material can be covered by available stock and/or incoming supply but the reservation/supply commitment is not complete':'One or more exact BOM items are not available or committed in sufficient quantity',issues:issuedReady?[]:partial?[`Current issued material limits output to ${outputLimit.maxBuildQty}/${outputLimit.requestedQty} requested unit(s). Issue/receive the remaining exact BOM quantity to remove the limiter.`]:supplyPlanReady?[]:lines.filter(x=>x.uncovered>0).map(x=>`${x.req.partNumber} Rev ${x.req.revision}: need ${x.req.requiredQty}; reserved/issued ${x.reserved}, committed incoming ${x.incoming}, unreserved stock ${x.available}, uncovered ${x.uncovered}`),lines,outputLimit,replenishments};
  };
...
(function installSharedProgrammeModelV161(P){
  ...
  P.planValidationV161=(state,pid,{siteId=null,notBefore=null,commit=true}={})=>{
    // REV 1.0.184+: Validation uses the same canonical programme planning API as Prototype.
...
(function installV176ValidationReporting(P){
  ...
  P.validationReportV161=(state,pid,{persist=false}={})=>{
    ...
    if(persist){state.validationReports=arr(state,'validationReports');for(const x of state.validationReports)if(x.programmeId===pid)x.current=false;const snap=clone(report);snap.evidence=(snap.evidence||[]).map(e=>e.type==='Test photo'?{...e,dataUrl:null}:e);snap.photoEvidence=(snap.photoEvidence||[]).map(e=>({...e,dataUrl:null}));state.validationReports.push(snap);if(p)p.reportRevision=report.revision;P.audit?.(state,'Validation report generated','Validation Programme',pid,'Current evidence',`${report.status} · Rev ${report.revision}`,`REV 1.0.184 controlled technical report · ${report.activities.filter(a=>a.kind==='Validation Test').length} tests · ${report.testFlow?.legs?.length||0} Test Legs · ISO/IEC 17025-aligned report structure.`)}
    return report;
  };
})(window.ProtoLab);

/* REV 1.0.184 — single-source Validation report approval controller.
