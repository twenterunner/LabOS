...
checks.append(('Versioned asset URLs bypass stale browser cache',bool(versioned) and all('?v=1.0.5' in r for r in versioned)))
...
checks.append(('Visible revision number is present in the application header','id="versionBadge"' in text and '>REV 1.0.5<' in text))
...
checks.append(('Standard process/test and competence master data drive planning','standardTests' in demo and 'competencies' in demo and 'Lab team competency matrix' in app and 'Standard Test Library' in app))
checks.append(('Planner uses confirmed route and qualified/calibrated resources','procAssess.route' in services and 'route.confirmed!==true' in core and "calibrationStatus==='Valid'" in services and 'x.competencies.includes(skillId)' in services))
