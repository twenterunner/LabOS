...
checks.append(('Versioned asset URLs bypass stale browser cache',bool(versioned) and all('?v=1.0.6' in r for r in versioned)))
...
checks.append(('Visible revision number is present in the application header','id="versionBadge"' in text and '>REV 1.0.6<' in text))
...
checks.append(('Planner uses confirmed route and qualified/calibrated resources','procAssess.route' in services and 'route.confirmed!==true' in core and 'P.equipmentReady' in services and 'P.staffQualification' in services))
