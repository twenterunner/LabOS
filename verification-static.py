...
checks.append(('Versioned asset URLs bypass stale browser cache',bool(versioned) and all('?v=1.0.7' in r for r in versioned)))
...
checks.append(('Visible revision number is present in the application header','id="versionBadge"' in text and '>REV 1.0.7<' in text))
checks.append(('Practical ten-step guided workflow is present',all(x in app for x in ['Request definition & submission','Material source & feasibility','Process route & test-method assessment','Lab feasibility, resource plan & committed timing','Control Plan, risk controls & special approvals','Build readiness','Serialise & execute digital traveller','Characterise, evaluate & disposition exceptions','Release approval','Deliver, retain records & feed learning'])))
checks.append(('AUTO-PLAN is downstream of material and process/test feasibility','AUTO-PLAN needs material feasibility first' in services and 'AUTO-PLAN needs process/test definition first' in services and 'A schedule is not credible yet' in app))
...
checks.append(('Standard process/test and competence master data drive planning','standardTests' in demo and 'competencies' in demo and 'Training certificates & staff coverage' in app and 'STANDARD TEST LIBRARY' in app))
...
checks.append(('Calibration certificate fields are controlled','Calibration certificate number' in app and 'Traceability / reference standard' in app and 'calibrationCertificates' in core))
checks.append(('Optimized readiness plan coordinates calibration maintenance and training','class ResourceCareService' in services and 'OPTIMIZED READINESS PLAN' in app and 'Readiness / non-build bookings' in app))
