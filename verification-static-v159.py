...
add('All JS/CSS assets use REV 1.0.59 cache busting',bool(versioned) and all('?v=1.0.59' in r for r in versioned))
add('Visible revision badge is REV 1.0.59','>REV 1.0.59<' in text)
add('Core version/schema are 1.0.59 / 29',"ProtoLab.VERSION = '1.0.59-poc'" in core and 'ProtoLab.SCHEMA_VERSION = 29' in core)
...
add('Gage R&R recommendation and explicit user decision are present','System recommendation: APPROVE' in app and 'Final decision belongs to the authorised reviewer' in app and 'userDecision' in app)
add('5S management KPI and trend are present','5S / WORKPLACE' in app and '5S workplace condition & trend' in app and 'fiveSKpiModelV1059' in app)
add('Calibration audit finding routes directly to exact resolver',"if(t.action==='calibration')return auditCalibrationResolutionModalV1059(key)" in app)
add('Audit evidence print mode is isolated and mobile safe','print-audit-evidence-now' in css and '.audit-evidence-row' in css)
add('Release sign-off uses stable pending approval IDs','data-approve-package-records' in app and 'JSON.stringify(g.pending.map(a=>a.id))' in app)
