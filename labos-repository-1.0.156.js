 ...
 static migrate(state){
  ...
  while(s.schemaVersion<P.SCHEMA_VERSION){
   ...
   if(s.schemaVersion===37){
    const wasDemo=P.isDemoDataset?.(s);s.settings=s.settings||{};
    P.ensureValidationModelV152?.(s);
    if(wasDemo)P.seedValidationDemoV152?.(s);
    s.migrationHistory=s.migrationHistory||[];
    if(!s.migrationHistory.some(x=>x.id==='schema37-to-38'))s.migrationHistory.push({id:'schema37-to-38',release:'1.0.152',at:P.now(),note:'Activated controlled Validation / DV / PV programme graph, requirement, DUT genealogy, execution, reporting and shared-resource planning data model.'});
    if(s.auditTrail)P.audit(s,'Schema migration','System','Validation / DV / PV','Schema 37','Schema 38','REV 1.0.152 explicit idempotent migration; Prototype records preserved.');
    s.dataVersion=wasDemo?'2026.09-demo-43-validation-domain':(s.dataVersion||'migrated');s.schemaVersion=38;continue;
   }
   throw new Error(`No migration available from schema ${s.schemaVersion}`);
  }
  P.ensureMaterialModel(s);P.ensurePlanningModel(s);P.ensureEnterpriseModel(s);P.ensureMultiLabModelV1099?.(s);P.ensureValidationModelV152?.(s);(s.requests||[]).forEach(r=>{P.assignRequestDefaultSiteV1099?.(s,r,{force:false});P.syncRequestFlowdown(s,r);});P.repairDemoWorkflowFixturesV1115?.(s);P.reconcileAllForecastsV1100?.(s);(s.serials||[]).forEach(sample=>P.ensureSampleEvidence(sample));P.repairDuplicateSamples(s);(s.deviations||[]).forEach(d=>{if(d.type==='NCR')d.type='Nonconformance';P.ensureQualityCase(d);});(s.requests||[]).forEach(r=>P.ensureApprovalRecords(s,r));return s;
 }
}
