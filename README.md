...
## REV 1.0.182 — generic planning & workflow regression recovery

REV 1.0.182 consolidates Prototype and Validation planning on one canonical `PlanningEngine` and repairs the Overall swim-lane view without project-specific routing rules. The same engine now drives AUTO-PLAN, manual feasible-slot search, local recovery, sister-lab comparison and receiving-lab revalidation.

Key regression fixes: six real lane filters in one row; All / Prototype / Validation domain filter; fixed project column plus independent draggable timeline; − / + / FIT; booking tap → Green/Yellow/Red manual slots; separate local **Optimize recovery** and network **Compare sister labs**; controlled sister-lab request → receiving Lab Manager accept/reject → replan/commit; Validation Archive Manager; selected-lab Audit and KPI context; KPI time-series line plots; material completion no longer turns green from a supply-date promise; and explicit Process & Methods confirmation before later workflow stages.

## REV 1.0.182 — complex Validation + engine-driven learning demo

REV 1.0.182 changes `V26-0205` into a completed 24-DUT **18 V brushless cordless drill/driver** Design Validation with 4 Test Legs, 31 tests, two split/branch/merge structures, realistic power-tool engineering records, setup photographs and a FINAL approved Validation Report.
...
## REV 1.0.182 — permanent Management Demo launcher
...
**Current prototype release: REV 1.0.182**  
**Data schema: 38**
...
## REV 1.0.182 · Validation technical-report upgrade
...
5. Confirm the header shows **REV 1.0.182**. Future deployments are checked automatically through the cache-bypassed `labos-version.json` handshake.
...
- `labos-core-1.0.182.js`, `labos-services-1.0.182.js`, `labos-repository-1.0.182.js`, `labos-demo-data-1.0.182.js`, `labos-app-1.0.182.js` — application runtime.
- `labos-styles-1.0.182.css` — application styling.
- `USER_MANUAL.html` — current task-based help.
- `DEMO_GUIDE_v1.0.182.md` — management-demo walkthrough and pitch capability map.
- `QA_REPORT_v1.0.182.md` — release verification and known browser-test limitation.
...
## REV 1.0.182 — management-demo showcase portfolio
...
- `DEMO_GUIDE_v1.0.182.md` provides a recommended walkthrough and pitch-oriented feature map.

## REV 1.0.182 — Prototype-linked Validation dependency audit hardening
