...
checks.push(['visible revision badge is populated',els['#versionBadge'].textContent==='REV 1.0.56']);
...
checks.push(['Capability & SPC is nested under shared Quality',appSource.includes("['capability','Capability & SPC','']")&&appSource.includes("if(view==='characterisation'){App.filters.qualityTab='capability';view='quality'}")]);
checks.push(['capability study exposes configuration and equipment filters',appSource.includes('id=\"capConfiguration\"')&&appSource.includes('id=\"capEquipment\"')&&appSource.includes('cap-study-scope')]);
