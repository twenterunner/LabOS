...
(async()=>{if(!domReady)throw new Error('DOMContentLoaded handler not registered');await domReady();await new Promise(r=>setTimeout(r,10));const checks=[
 ...
 ['navigation markup rendered',/Prototype/.test(els['#mainNav'].innerHTML)],
...
checks.push(['visible revision badge is populated',els['#versionBadge'].textContent==='REV 1.0.57']);
...
checks.push(['future workstreams are visible but explicitly non-operational',appSource.includes('Future</span>')&&appSource.includes('POC boundary:')&&appSource.includes('renderValidationFuture')&&appSource.includes('renderFailureAnalysisFuture')]);
checks.push(['Capability & SPC is removed and legacy navigation redirects to MSA',!appSource.slice(appSource.indexOf('REV 1.0.57')).includes("['capability','Capability & SPC'")&&appSource.includes("if(view==='characterisation'){App.filters.qualityTab='msa';view='quality'}")]);
checks.push(['MSA study exposes explicit process, test and equipment links',appSource.includes('id=\"grrProcess\"')&&appSource.includes('id=\"grrTest\"')&&appSource.includes('id=\"grrEquipment\"')&&appSource.includes('msa-link-grid')]);
