...
class PlanningEngine{
  ...
  optimizeRecovery(id){
    const e=this.entity(id);if(!e)throw new Error('Programme was not found.');
    const siteId=this.entitySite(e)||this.activeLabId(),currentForecast=isoDay(e.forecastDate),required=isoDay(e.requiredDate),baselineLate=required&&currentForecast?Math.max(0,daysBetween(required,currentForecast)):null;
    let local=null,error=null;try{local=this.plan(id,{siteId,notBefore:e.dutAvailableDate||e.planningNotBefore||null})}catch(err){error={reason:err.message,code:err.code||'PLANNING_BLOCKED'}}
    const rows=[];if(local?.ok){const newLate=required&&local.forecastDate?Math.max(0,daysBetween(required,local.forecastDate)):null,improvementDays=currentForecast&&local.forecastDate?Math.max(0,daysBetween(local.forecastDate,currentForecast)):0,improves=!currentForecast||((newLate??0)<(baselineLate??0)||String(local.forecastDate)<String(currentForecast));rows.push({...local,name:this.labName(siteId),current:true,currentForecast,baselineLateDays:baselineLate,newLateDays:newLate,improvementDays,improves,kind:'local-recovery'})}
    return {programmeId:id,domain:this.domainOf(id),currentSiteId:siteId,currentForecast,requiredDate:required||null,rows,improvements:rows.filter(x=>x.improves),localError:error,networkAvailable:this.internalLabs().some(l=>l.id!==siteId)}
  }
  ...
  requestSisterLab(id,siteId,{reason='Sister-lab execution requested from Planning'}={}){
    const w=this.entity(id);if(!w)throw new Error('Programme was not found.');const fromSiteId=this.entitySite(w)||this.activeLabId();if(!siteId||siteId===fromSiteId)throw new Error('Select a different internal laboratory for a sister-lab request.');
    const lab=this.internalLabs().find(x=>x.id===siteId);if(!lab)throw new Error('The target sister laboratory is not an active internal laboratory.');
    const candidate=this.plan(id,{siteId,notBefore:w.dutAvailableDate||w.planningNotBefore||null});if(!candidate?.ok)throw new Error('The sister-lab candidate is no longer feasible.');
    this.state.networkTransfers=Array.isArray(this.state.networkTransfers)?this.state.networkTransfers:[];const existing=this.state.networkTransfers.find(x=>x.programmeId===id&&x.targetSiteId===siteId&&['Requested','Pending'].includes(x.status));if(existing)return existing;
    const tr={id:P.uid?P.uid('XFER'):`XFER-${Date.now()}`,requestId:id,programmeId:id,domain:this.domainOf(id),scope:'programme',sourceSiteId:fromSiteId,fromSiteId,targetSiteId:siteId,toSiteId:siteId,status:'Requested',requestedAt:P.now?.()||new Date().toISOString(),requestedBy:this.state.identity?.name||'Planner',rationale:reason,candidateForecast:candidate.forecastDate||null,candidateBookingCount:candidate.bookingCount||0};this.state.networkTransfers.unshift(tr);P.audit?.(this.state,'Sister-lab execution requested',tr.domain==='validation'?'Validation Programme':'Request',id,this.labName(fromSiteId),this.labName(siteId),`${reason} · forecast ${candidate.forecastDate||'—'} · ${candidate.bookingCount||0} booking(s)`);return tr
  }
  transferRequests(siteId=this.activeLabId()){return (this.state.networkTransfers||[]).filter(x=>x.scope==='programme'&&['Requested','Pending'].includes(x.status)&&(x.sourceSiteId===siteId||x.targetSiteId===siteId||x.fromSiteId===siteId||x.toSiteId===siteId))}
  acceptSisterLab(transferId,{reason='Receiving Lab Manager accepted and revalidated the constrained plan'}={}){
    const tr=(this.state.networkTransfers||[]).find(x=>x.id===transferId);if(!tr)throw new Error('Transfer request was not found.');if(!['Requested','Pending'].includes(tr.status))throw new Error('Transfer request is no longer pending.');const id=tr.programmeId||tr.requestId,target=tr.targetSiteId||tr.toSiteId,w=this.entity(id);if(!w)throw new Error('Programme was not found.');
    const result=this.plan(id,{siteId:target,notBefore:w.dutAvailableDate||w.planningNotBefore||null});const ctr=(result.state.networkTransfers||[]).find(x=>x.id===transferId);if(ctr){ctr.status='Accepted';ctr.acceptedAt=P.now?.()||new Date().toISOString();ctr.acceptedBy=this.state.identity?.name||'Lab Manager';ctr.acceptanceReason=reason;ctr.actualForecast=result.forecastDate||null}
    const out=this.commitPlan(result,{action:'Sister-lab transfer accepted and plan committed',reason});const live=(this.state.networkTransfers||[]).find(x=>x.id===transferId);if(live&&live.status!=='Accepted'){live.status='Accepted';live.acceptedAt=P.now?.()||new Date().toISOString();live.acceptedBy=this.state.identity?.name||'Lab Manager'}return out
  }
  rejectSisterLab(transferId,{reason='Receiving laboratory rejected the transfer request'}={}){const tr=(this.state.networkTransfers||[]).find(x=>x.id===transferId);if(!tr)throw new Error('Transfer request was not found.');tr.status='Rejected';tr.rejectedAt=P.now?.()||new Date().toISOString();tr.rejectedBy=this.state.identity?.name||'Lab Manager';tr.rejectionReason=reason;P.audit?.(this.state,'Sister-lab execution request rejected',tr.domain==='validation'?'Validation Programme':'Request',tr.programmeId||tr.requestId,this.labName(tr.sourceSiteId||tr.fromSiteId),this.labName(tr.targetSiteId||tr.toSiteId),reason);return tr}
  kpiScopedState(siteId=this.activeLabId()){return this.auditScopedState(siteId)}
...
P.requestSisterLabV182=(state,id,siteId,options={})=>new PlanningEngine(state).requestSisterLab(id,siteId,options);
P.acceptSisterLabV182=(state,transferId,options={})=>new PlanningEngine(state).acceptSisterLab(transferId,options);
P.rejectSisterLabV182=(state,transferId,options={})=>new PlanningEngine(state).rejectSisterLab(transferId,options);
P.scopeLabKpiV182=(state,siteId)=>new PlanningEngine(state).kpiScopedState(siteId);
