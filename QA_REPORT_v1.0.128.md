# LabOS REV 1.0.128 — Controlled Business Units + Planning/KPI Regression QA

**Audit date:** 15 September 2026  
**Audit target:** `ProtoLabOS_Prototype_Build_POC_v1.0.128_WEB.zip`  
**Regression base:** REV 1.0.126  
**Accepted historical reference:** REV 1.0.98  
**Schema:** 35

## Executive result
REV 1.0.128 introduces **controlled Business Unit master data** in Configuration. Project Teams no longer accept a free-format Business Unit string: they select a governed Business Unit record with a stable ID, optional short code and active/inactive state. Existing free-text values migrate automatically, and requests retain a Business Unit snapshot so historical selected-period KPI allocation remains stable when the organization later changes.

The fresh demo portfolio now spans five Business Units. The change is additive to the REV 1.0.126 manual/AUTO-PLAN and selected-period KPI behavior; no IndexedDB schema reset is required.


## 0. Inherited REV 1.0.126 manual/AUTO-PLAN regression
...
## 1. Inherited manual-replan transaction regression
### Symptom
...
## 2. Replan rationale — before vs REV 1.0.128
### Before
...
## 4. Controlled Business Units + allocation
REV 1.0.128 replaces the Project Team free-text Business Unit field with controlled organization master data.

Configuration now contains **Business Units** with stable ID, name, optional short code and active/inactive state. A Project Team selects an active Business Unit from a dropdown. Deactivation is blocked while an active Project Team still references the unit. Existing free-text values migrate into master records automatically; no schema reset is required.

Requests retain a Business Unit ID/name snapshot so historical KPI allocation does not silently change if a Project Team is later reassigned. The KPI calculation resolves that snapshot first and uses the Project Team only as a compatibility fallback.

For active-lab bookings within the selected period:
...
Fresh demo Business Units and request counts:
- Professional Power Tools: 5 requests;
- Consumer & DIY: 5;
- Outdoor & Garden: 5;
- Industrial Solutions: 5;
- Battery & Energy Systems: 4.

Migration tests confirm that a legacy free-text `Legacy BU` becomes a stable master record, a master rename updates display names by stable ID, and moving a Project Team to another configured Business Unit does not rewrite the already-snapshotted historical request.
...
- 300 feasible;
- 38 blocked with structured planner results;
...
## REV 1.0.128 addendum — Business Unit upgrade migration

### User-visible defect
A deployed database upgraded from REV 1.0.126 could show a 100% `Professional Power Tools` KPI allocation at every lab even though REV 1.0.127 contained diverse fresh-demo Business Unit data.

### Root cause
The browser correctly retained the user's existing IndexedDB state instead of reseeding it. All older demo Project Teams/requests carried the single legacy free-text value `Power Tools`, which the new controlled-BU migration correctly normalized to `Professional Power Tools`. The fresh-demo diversification code only runs while creating/resetting demo data, so it never touched that already-persisted portfolio.

### Repair
REV 1.0.128 performs an in-place, guarded repair only when all of the following are true: the state is marked as the demo dataset, at least 20 of the canonical `P26-1001`–`P26-1024` examples are present, and their current BU snapshots are still collapsed to the legacy Power Tools / Professional Power Tools value. Known Project Team names are mapped to the five controlled demo Business Units and only those canonical demo request snapshots are updated. Already-diverse states and non-demo/user portfolios are left unchanged.

### Verification
- Fresh REV 1.0.128 state: 5 distinct Business Units across all 24 demo requests.
- Simulated REV 1.0.126/1.0.127 persisted upgrade with all 24 demo requests collapsed to Professional Power Tools: migration result = 5 distinct Business Units.
- Twente demo requests span all 5 Business Units; Stuttgart spans all 5; Detroit spans all 5.
- Default last-13-calendar-weeks booked-hours allocation after migration: Twente = 4 BUs represented, Stuttgart = 4, Detroit = 3.
- `validateInvariants`: 0 findings after migration.
- `planningIntegrityAudit`: 0 findings after migration.
