(function(){
  ...
  ProtoLab.networkMetricsV1099 = state => {
    ...
    return {sisterLabBuilds:rows.filter(x=>x.status==='Accepted'&&x.scope!=='task'&&x.fromSiteId!==x.toSiteId).length,sisterLabTests:rows.filter(x=>x.status==='Accepted'&&x.scope==='task'&&x.fromSiteId!==x.toSiteId).length,internallyRetainedHours:rows.filter(x=>x.status==='Accepted').reduce((n,x)=>n+Number(x.plannedHours||0),0),outsourcingAvoided:rows.filter(x=>x.status==='Accepted').reduce((n,x)=>n+Number(x.avoidedExternalCost||0),0),externalSpend:rows.filter(x=>x.status==='External').reduce((n,x)=>n+Number(x.externalCost||0),0)};
  };
