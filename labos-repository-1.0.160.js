...
class MigrationService{
 static migrate(state){
  ...
  while(s.schemaVersion<P.SCHEMA_VERSION){
   ...
   // planning/governance revisions, but the explicit migration links were omitted.
   // Keep the transitions deliberately idempotent and data-preserving: normalise
   ...
   if(s.schemaVersion===37){
    const wasDemo=P.isDemoDataset?.(s);s.settings=s.settings||{};
    ['validationProgrammes','validationRequirements','validationLegs','validationNodes','validationEdges','validationResults','validationReports','validationSpecSources','validationDevelopment','validationDuts','validationAudit','standardTestCandidates'].forEach(k=>{if(!Array.isArray(s[k]))s[k]=[];});
    s.settings.migrationHistory=Array.isArray(s.settings.migrationHistory)?s.settings.migrationHistory:[];
    if(!s.settings.migrationHistory.some(x=>x?.id==='schema37-to-38-v160'))s.settings.migrationHistory.push({id:'schema37-to-38-v160',at:P.now?.()||new Date().toISOString(),release:'1.0.160',note:'Restored schema-38 compatibility after the REV 1.0.160 UI rebuild. Existing Prototype and Validation records are preserved; Validation collections are initialised only when absent.'});
    if(s.auditTrail)P.audit(s,'Schema compatibility restored','System','Data model','Schema 37 REV 1.0.159 rebuild','Schema 38','REV 1.0.160 preserves existing data and restores compatibility with schema-38 browser state.');
    s.dataVersion=wasDemo?'2026.09-demo-41-schema38-compat':(s.dataVersion||'migrated');s.schemaVersion=38;continue;
   }
   throw new Error(`No migration available from schema ${s.schemaVersion}`);
  }
  P.ensureMaterialModel(s);P.ensurePlanningModel(s);P.ensureEnterpriseModel(s);P.ensureMultiLabModelV1099?.(s);(s.requests||[]).forEach(r=>{P.assignRequestDefaultSiteV1099?.(s,r,{force:false});P.syncRequestFlowdown(s,r);});P.repairDemoWorkflowFixturesV1115?.(s);P.reconcileAllForecastsV1100?.(s);(s.serials||[]).forEach(sample=>P.ensureSampleEvidence(sample));P.repairDuplicateSamples(s);(s.deviations||[]).forEach(d=>{if(d.type==='NCR')d.type='Nonconformance';P.ensureQualityCase(d);});(s.requests||[]).forEach(r=>P.ensureApprovalRecords(s,r));return s;
 }
}
