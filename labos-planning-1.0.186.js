/* LabOS REV 1.0.186 — canonical planning regression recovery */
(function(P){
'use strict';
if(!P)return;
...
class PlanningEngine{
  ...
  _prototypePlan(id,{siteId,notBefore=null,manualConstraint=null}={}){
    ...
    let work=P.siteScopedStateV1099?P.siteScopedStateV1099(base,sid,{includeRequestId:id,includeNetworkTasks:false}):base;const r=(work.requests||[]).find(x=>x.id===id);if(!r)throw new Error('Prototype build is outside the selected planning context.');r.executionSiteId=sid;if(notBefore)r.planningNotBefore=notBefore;
  ...
  _validationPlan(id,{siteId,notBefore=null,manualConstraint=null}={}){
    const base=clone(this.state);P.ensureMultiLabModelV1099?.(base);P.ensureProgrammeModelV161?.(base,{seedDemo:false});const p=(base.validationProgrammes||[]).find(x=>x.id===id);if(!p)throw new Error('Validation programme was not found.');if(!P.validationActivitiesV161?.(base,id)?.length)throw new Error('Generate the Validation programme before resource planning.');const sid=siteId||p.executionSiteId||p.homeSiteId||base.settings?.activeLabId||base.settings?.primaryLabId;p.executionSiteId=sid;if(notBefore)p.planningNotBefore=notBefore;
    const synthetic=P.validationSyntheticRequestV161(base,p);synthetic.executionSiteId=sid;synthetic.homeSiteId=p.homeSiteId||sid;synthetic.taskSiteOverridesV1170={};if(notBefore)synthetic.planningNotBefore=notBefore;
    ...
    base.requests=(base.requests||[]).filter(x=>x.id!==id);base.requests.push(synthetic);
    // REV 1.0.186: solve the whole Validation programme strictly inside the selected
    // laboratory. Existing remote bookings stay as provenance/history and may not
    // leak equipment or people into a sister-lab feasibility result.
    let work=P.siteScopedStateV1099?P.siteScopedStateV1099(base,sid,{includeRequestId:id,includeNetworkTasks:false}):base;P.ensurePlanningModel?.(work);new P.PlannerService().autoPlan(work,id);
    let enterprise=P.mergeSitePlanningStateV1099?P.mergeSitePlanningStateV1099(base,work,sid):work;
    const generated=(enterprise.bookings||[]).filter(b=>b.requestId===id&&!historical(b));for(const b of generated)Object.assign(b,{domain:'Validation',programmeType:'validation',validationProgrammeId:id,siteId:b.siteId||sid});
    const forecast=this._forecast(enterprise,id),liveP=(enterprise.validationProgrammes||[]).find(x=>x.id===id);if(liveP){liveP.forecastDate=forecast;liveP.executionSiteId=sid;liveP.status='Planned';liveP.lastPlannedAt=P.now();liveP.lastPlannedBy=enterprise.identity?.name||'Lab Planner';}
    const normalize=v=>String(v||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim(),byName=n=>generated.find(b=>normalize(b.stepName).includes(normalize(n))||normalize(n).includes(normalize(String(b.stepName||'').replace(/^test\s*·\s*/i,''))));
    for(const a of P.validationActivitiesV161(enterprise,id)||[]){const b=byName(a.name);if(b){a.plannedStart=b.start;a.plannedEnd=b.end;a.equipmentId=b.equipmentId||null;a.staffId=b.staffId||null;a.siteId=b.siteId||sid}}
    // Validation remains first-class persistent data; the synthetic Prototype-shaped
    // request exists only while the common constraint solver is running.
    enterprise.requests=(enterprise.requests||[]).filter(x=>x.id!==id);P.syncValidationActionsV161?.(enterprise);return this._result(enterprise,id,'validation',sid);
  }
  planningHorizonDays(){const configured=Number(this.state.settings?.planningHorizonDays||180);return Number.isFinite(configured)?Math.max(30,Math.min(365,configured)):180}
  _enforceHorizon(result){const start=P.todayISO?.()||new Date().toISOString().slice(0,10),forecast=isoDay(result?.forecastDate),limit=addDays(start,this.planningHorizonDays());if(forecast&&forecast>limit){const e=new Error(`No complete feasible plan exists inside the ${this.planningHorizonDays()}-day planning horizon. Latest feasible completion would be ${forecast}; use recovery, sister-lab, reprioritisation or external-capacity options instead of creating a multi-year plan.`);e.code='PLANNING_HORIZON_EXCEEDED';e.forecastDate=forecast;e.horizonEnd=limit;e.programmeId=result?.programmeId;throw e}return result}
  plan(id,{siteId=null,notBefore=null,manualConstraint=null}={}){const d=this.domainOf(id);if(!d)throw new Error('Planning programme was not found.');const r=d==='validation'?this._validationPlan(id,{siteId,notBefore,manualConstraint}):this._prototypePlan(id,{siteId,notBefore,manualConstraint});return this._enforceHorizon(r)}
  ...
  _taskContext(programmeId,stepId){
    const domain=this.domainOf(programmeId),entity=this.entity(programmeId);if(!domain||!entity)return {ok:false,error:'Programme was not found.'};
    let tasks=[];try{if(domain==='prototype')tasks=P.planningTasksForRequest?.(this.state,entity)||[];else{const synthetic=P.validationSyntheticRequestV161?.(this.state,entity);tasks=synthetic?P.planningTasksForRequest?.(this.state,synthetic)||[]:[]}}catch(_){tasks=[]}
    const task=tasks.find(t=>t.id===stepId);return task?{ok:true,domain,entity,task,home:this.entitySite(entity)}:{ok:false,domain,entity,error:'This activity is not an active canonical planning step.'};
  }
  planTaskAtSite(programmeId,stepId,toSiteId){
    const ctx=this._taskContext(programmeId,stepId);if(!ctx.ok)throw Object.assign(new Error(ctx.error),{code:'TASK_NOT_ROUTABLE'});const lab=this.internalLabs().find(l=>l.id===toSiteId);if(!lab||toSiteId===ctx.home)throw Object.assign(new Error('Select a different active internal sister lab.'),{code:'INVALID_SISTER_LAB'});
    const candidate=clone(this.state);P.ensureMultiLabModelV1099?.(candidate);let syntheticAdded=false;
    let work=(candidate.requests||[]).find(x=>x.id===programmeId);
    if(ctx.domain==='validation'){
      P.ensureProgrammeModelV161?.(candidate,{seedDemo:false});const p=(candidate.validationProgrammes||[]).find(x=>x.id===programmeId);if(!p)throw new Error('Validation programme was not found.');
      work=P.validationSyntheticRequestV161(candidate,p);candidate.requests=(candidate.requests||[]).filter(x=>x.id!==programmeId);candidate.requests.push(work);syntheticAdded=true;
    }
    if(!work)throw new Error('Planning work item was not found.');work.taskSiteOverridesV1170={...(work.taskSiteOverridesV1170||{}),[stepId]:toSiteId};
    new P.PlannerService().autoPlan(candidate,programmeId);
    const bookings=(candidate.bookings||[]).filter(b=>b.requestId===programmeId&&!historical(b)),moved=bookings.find(b=>b.stepId===stepId);if(!moved||this._bookingSiteFromState(candidate,moved)!==toSiteId)throw Object.assign(new Error('The canonical solver could not place the selected activity at the requested sister lab.'),{code:'TASK_SITE_ASSIGNMENT_FAILED'});
    const forecast=this._forecast(candidate,programmeId);if(ctx.domain==='validation'){
      const p=(candidate.validationProgrammes||[]).find(x=>x.id===programmeId);if(p){p.forecastDate=forecast;p.status='Planned';p.lastPlannedAt=P.now?.()||new Date().toISOString()}
      candidate.requests=(candidate.requests||[]).filter(x=>x.id!==programmeId);
    }
    const result=this._result(candidate,programmeId,ctx.domain,ctx.home);result.taskTransfer={stepId,toSiteId,taskKind:ctx.task.kind,taskName:ctx.task.name,movedBooking:moved};return this._enforceHorizon(result);
  }
  _bookingSiteFromState(state,b){if(b?.siteId)return b.siteId;const e=(state.equipment||[]).find(x=>x.id===b?.equipmentId),st=(state.staff||[]).find(x=>x.id===b?.staffId);return e?.siteId||st?.siteId||(state.requests||[]).find(x=>x.id===b?.requestId)?.executionSiteId||(state.validationProgrammes||[]).find(x=>x.id===b?.requestId)?.executionSiteId||null}
  compareTaskLabs(programmeId,stepId){
    const ctx=this._taskContext(programmeId,stepId);if(!ctx.ok)return {ok:false,programmeId,stepId,error:ctx.error,rows:[]};const rows=[];
    for(const lab of this.internalLabs().filter(l=>l.id!==ctx.home)){try{const sc=this.planTaskAtSite(programmeId,stepId,lab.id),m=sc.taskTransfer.movedBooking;rows.push({ok:true,siteId:lab.id,lab,programmeId,stepId,taskKind:ctx.task.kind,taskName:ctx.task.name,taskDate:isoDay(m.start),forecast:sc.forecastDate,forecastDate:sc.forecastDate,equipmentId:m.equipmentId||null,staffId:m.staffId||null,plannedHours:Number(m.durationHours||0),state:sc.state,scenario:sc})}catch(e){rows.push({ok:false,siteId:lab.id,lab,programmeId,stepId,taskKind:ctx.task.kind,taskName:ctx.task.name,error:e.message,reason:e.message,code:e.code||'PLANNING_BLOCKED',failure:{error:e.message,code:e.code||'PLANNING_BLOCKED'}})}}
    return {ok:rows.some(x=>x.ok),programmeId,requestId:programmeId,stepId,taskKind:ctx.task.kind,taskName:ctx.task.name,homeSiteId:ctx.home,currentSiteId:ctx.home,rows,generatedAt:P.now?.()||new Date().toISOString()};
  }
  requestTaskTransfer(programmeId,stepId,toSiteId,{reason='Sister-lab operation requested',actor=null}={}){
    const pack=this.compareTaskLabs(programmeId,stepId),candidate=pack.rows.find(x=>x.ok&&x.siteId===toSiteId);if(!candidate)throw new Error(pack.rows.find(x=>x.siteId===toSiteId)?.reason||'The receiving lab no longer has a complete feasible operation plan.');
    const user=actor||this.state.identity||{},rec={id:P.uid?P.uid('STEPREQ'):`STEP-${Date.now()}`,governanceModel:'unified-programme',scope:'task',transferType:'task',domain:this.domainOf(programmeId),requestId:programmeId,programmeId,stepId,stepName:pack.taskName,taskKind:pack.taskKind,fromSiteId:pack.homeSiteId,toSiteId,status:'Requested',requestStage:'receiver-acceptance',requestedAt:P.now?.()||new Date().toISOString(),requestedBy:user.name||'Planner',requestedByRole:user.role||null,rationale:String(reason||'').trim(),requestedForecast:candidate.forecastDate,requestedTaskDate:candidate.taskDate,requestedPlannedHours:Number(candidate.plannedHours||0)};
    this.transferRequests().push(rec);P.audit?.(this.state,'Sister-lab operation requested',rec.domain==='validation'?'Validation Programme':'Request',programmeId,rec.fromSiteId,toSiteId,`${rec.stepName}; ${rec.rationale}; live plan unchanged pending receiver acceptance`);return rec;
  }
  acceptTaskTransfer(recordId,{reason='Receiving laboratory accepted after current-capacity review',actor=null}={}){
    const rec=this.transferRecord(recordId);if(!rec||rec.status!=='Requested'||rec.scope!=='task')throw new Error('This operation transfer is no longer awaiting a decision.');const result=this.planTaskAtSite(rec.requestId,rec.stepId,rec.toSiteId),user=actor||this.state.identity||{};
    this._replaceState(result.state);const live=this.transferRecord(recordId)||rec;Object.assign(live,{status:'Accepted',requestStage:'scheduled',acceptedAt:P.now?.()||new Date().toISOString(),acceptedBy:user.name||'Receiver',acceptedByRole:user.role||null,receiverDecisionReason:String(reason||'').trim(),appliedForecast:result.forecastDate,appliedBookingCount:result.bookingCount});
    const w=this.entity(rec.requestId);if(w){w.taskSiteOverridesV1170={...(w.taskSiteOverridesV1170||{}),[rec.stepId]:rec.toSiteId}}
    P.audit?.(this.state,'Sister-lab operation accepted by receiving lab',live.domain==='validation'?'Validation Programme':'Request',live.requestId,live.fromSiteId,live.toSiteId,`${live.stepName}; ${live.receiverDecisionReason}; canonical plan revalidated and applied`);return live;
  }
  advanceTransfer(recordId,status,{reason='',actor=null,evidenceRef=null}={}){
    const rec=this.transferRecord(recordId);if(!rec)throw new Error('Transfer record was not found.');const allowed={Accepted:'Scheduled',Scheduled:'In Execution','In Execution':'Results Returned','Results Returned':'Closed'},next=allowed[rec.status];if(status!==next)throw new Error(`Transfer ${rec.status} can only advance to ${next||'no further status'}.`);const user=actor||this.state.identity||{},now=P.now?.()||new Date().toISOString();rec.status=status;rec.requestStage=norm(status).replace(/\s+/g,'-');rec.lastUpdatedAt=now;rec.lastUpdatedBy=user.name||'User';if(reason)rec.lastStatusReason=String(reason);if(evidenceRef)rec.resultEvidenceRef=evidenceRef;if(status==='Scheduled')rec.scheduledAt=now;if(status==='In Execution')rec.executionStartedAt=now;if(status==='Results Returned')rec.resultsReturnedAt=now;if(status==='Closed')rec.closedAt=now;P.audit?.(this.state,`Sister-lab transfer ${status.toLowerCase()}`,rec.domain==='validation'?'Validation Programme':'Request',rec.requestId,allowed[Object.keys(allowed).find(k=>allowed[k]===status)]||'—',status,`${reason||'Controlled lifecycle transition'}${evidenceRef?` · evidence ${evidenceRef}`:''}`);return rec;
  }
...
P.compareProgrammeTaskLabsV186=(state,id,stepId)=>new PlanningEngine(state).compareTaskLabs(id,stepId);
P.requestProgrammeTaskTransferV186=(state,id,stepId,toSiteId,options={})=>new PlanningEngine(state).requestTaskTransfer(id,stepId,toSiteId,options);
P.acceptProgrammeTaskTransferV186=(state,recordId,options={})=>new PlanningEngine(state).acceptTaskTransfer(recordId,options);
P.advanceProgrammeTransferV186=(state,recordId,status,options={})=>new PlanningEngine(state).advanceTransfer(recordId,status,options);
