...
add('All JS/CSS assets use REV 1.0.53 cache busting',bool(versioned) and all('?v=1.0.53' in r for r in versioned))
add('Visible revision badge is REV 1.0.53','>REV 1.0.53<' in text)
add('Core version/schema are 1.0.53 / 26',"ProtoLab.VERSION = '1.0.53-poc'" in core and 'ProtoLab.SCHEMA_VERSION = 26' in core)
add('Schema-25 migration exists','if(s.schemaVersion===25)' in repo and 's.schemaVersion=26' in repo)
