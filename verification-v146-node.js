...
ok('Version is REV 1.0.46 / schema 21',()=>P.VERSION==='1.0.46-poc'&&P.SCHEMA_VERSION===21);
...
ok('User manual and shell identify REV 1.0.46',()=>/REV 1\.0\.46/.test(index)&&/REV 1\.0\.46/.test(manual)&&/styles\.css\?v=1\.0\.46/.test(index));
...
(async()=>{
 ...
 fs.writeFileSync(path.join(root,'VERIFICATION_RESULTS_v1.0.46.json'),JSON.stringify({version:P.VERSION,passed:pass,failed:fail,total:pass+fail,rows},null,2));
