(function(){
  ...
  ProtoLab.VERSION = '1.0.109-poc';
  ProtoLab.SCHEMA_VERSION = 35;
  ...
  ProtoLab.ensureApprovalRecords = (state,r) => {
    ...
    const gate=ProtoLab.GATES.indexOf(r.currentGate||r.status), readinessGate=ProtoLab.GATES.indexOf('BUILD READINESS REVIEW'), releaseGate=ProtoLab.GATES.indexOf('ENGINEERING REVIEW');
    ...
    // No approval is needed to create or submit a build request. Readiness approvals are instantiated only when the build actually reaches readiness.
    if(gate<readinessGate){state.approvals=state.approvals.filter(a=>a.requestId!==r.id||a.status==='Approved'||!['Product Safety','Build Readiness'].includes(a.type));}
    else if(r.productSafety&&!state.approvals.some(a=>a.requestId===r.id&&a.type==='Product Safety')){const done=gate>=ProtoLab.GATES.indexOf('RELEASED');state.approvals.push({id:ProtoLab.uid('APR'),requestId:r.id,type:'Product Safety',role:'Product Safety Representative',person:user('product_safety')?.name||'Product Safety Representative',status:done?'Approved':'Pending',timestamp:done?ProtoLab.now():null,stage:'readiness',comment:done?'Migrated/seeded evidence assumed complete for already released build.':'Review product-safety relevance, special characteristics and required evidence before physical build readiness.'});}
  ...
  ProtoLab.reconcilePlanningBookings = (state,requestOrId,{removeObsolete=true}={}) => {const r=typeof requestOrId==='string'?(state.requests||[]).find(x=>x.id===requestOrId):requestOrId;if(!r)return {removed:[],retained:[],tasks:[]};const tasks=ProtoLab.planningTasksForRequest(state,r),byId=new Map(tasks.map(t=>[t.id,t])),removed=[],retained=[];state.bookings=Array.isArray(state.bookings)?state.bookings:[];state.bookings=state.bookings.filter(b=>{if(b.requestId!==r.id||ProtoLab.isHistoricalPlanningBooking(b))return true;const t=byId.get(b.stepId),ok=t&&ProtoLab.planningTaskCompatible(t,b);if(ok){b.taskDefinitionKey=t.definitionKey;b.taskKind=t.kind;retained.push(b);return true}if(removeObsolete){removed.push(b);return false}return true});if(removed.length){const removedIds=new Set(removed.map(x=>x.id));state.resourceCareBookings=(state.resourceCareBookings||[]).filter(c=>c.sourceRequestId!==r.id||c.portfolioOwned===true||ProtoLab.isHistoricalPlanningBooking(c)||!c.autoGenerated);if(state.auditTrail)ProtoLab.audit(state,'Stale planning bookings reconciled','Request',r.id,removed.map(x=>x.stepName||x.stepId).join('; '),'Removed from active plan','Planning definition changed; historical execution evidence retained.')}return {removed,retained,tasks};};
  ...
  ProtoLab.processPlanningAssessment = (state,r) => {
    ...
    (route?.steps||[]).forEach(s=>{const proc=(state.processes||[]).find(p=>p.id===s.processId);if(s.type==='standard'){
      if(!proc)issues.push(`${s.name}: process definition is missing.`);else{
        ...
        if(!ProtoLab.planningCapabilityForProcess(state,r,proc)&&!/material receipt|cleaning|assembly|packaging/i.test(s.name||''))issues.push(`${s.name}: equipment capability could not be inferred or defined.`);if(!ProtoLab.planningSkillForProcess(proc))issues.push(`${s.name}: required competency could not be inferred or defined.`);
      }
  ...
  ProtoLab.siteScopedStateV1099 = (state,siteId,{includeRequestId=null}={}) => {
    ProtoLab.ensureMultiLabModelV1099(state);
    ...
    const requestIds=new Set((out.requests||[]).filter(r=>ProtoLab.requestExecutionSiteIdV1099(out,r)===sid).map(r=>r.id));
    if(includeRequestId)requestIds.add(includeRequestId);
    out.requests=(out.requests||[]).filter(r=>requestIds.has(r.id));
    const eqIds=new Set((out.equipment||[]).filter(e=>e.siteId===sid).map(e=>e.id));
    const staffIds=new Set((out.staff||[]).filter(s=>s.siteId===sid).map(s=>s.id));
    const matIds=new Set((out.materials||[]).filter(m=>m.siteId===sid).map(m=>m.id));
    out.equipment=(out.equipment||[]).filter(e=>eqIds.has(e.id));out.staff=(out.staff||[]).filter(s=>staffIds.has(s.id));out.materials=(out.materials||[]).filter(m=>matIds.has(m.id));
    ...
    out.allocations=(out.allocations||[]).filter(a=>requestIds.has(a.requestId)&&matIds.has(a.materialId));
    out.bookings=(out.bookings||[]).filter(b=>requestIds.has(b.requestId)&&(!b.equipmentId||eqIds.has(b.equipmentId))&&(!b.staffId||staffIds.has(b.staffId)));
    out.resourceCareBookings=(out.resourceCareBookings||[]).filter(b=>b.siteId===sid||eqIds.has(b.equipmentId)||staffIds.has(b.staffId)||requestIds.has(b.sourceRequestId));
    out.trainingCertificates=(out.trainingCertificates||[]).filter(c=>staffIds.has(c.staffId));out.calibrationCertificates=(out.calibrationCertificates||[]).filter(c=>eqIds.has(c.equipmentId));
    out.planningEvents=(out.planningEvents||[]).filter(ev=>ev.siteId===sid||eqIds.has(ev.equipmentId)||staffIds.has(ev.staffId));
    out.settings=out.settings||{};out.settings.activeLabId=sid;out.settings._planningSiteIdV1099=sid;
    return out;
  };
  ...
  ProtoLab.networkMetricsV1099 = state => {
    ProtoLab.ensureMultiLabModelV1099(state);const rows=state.networkTransfers||[];
    return {sisterLabBuilds:rows.filter(x=>x.status==='Accepted'&&x.fromSiteId!==x.toSiteId).length,internallyRetainedHours:rows.filter(x=>x.status==='Accepted').reduce((n,x)=>n+Number(x.plannedHours||0),0),outsourcingAvoided:rows.filter(x=>x.status==='Accepted').reduce((n,x)=>n+Number(x.avoidedExternalCost||0),0),externalSpend:rows.filter(x=>x.status==='External').reduce((n,x)=>n+Number(x.externalCost||0),0)};
  };
  ...
  ProtoLab.validateInvariants=state=>{
    ...
    for(const b of state.bookings||[]){const r=reqById.get(b.requestId);if(!r)continue;const sid=ProtoLab.requestExecutionSiteIdV1099(state,r),eq=b.equipmentId&&eqById.get(b.equipmentId),st=b.staffId&&stById.get(b.staffId);if(eq&&eq.siteId!==sid)out.push(`${r.id}: booking ${b.id||b.stepId} uses equipment from ${eq.siteId} while execution site is ${sid}`);if(st&&st.siteId!==sid)out.push(`${r.id}: booking ${b.id||b.stepId} uses staff from ${st.siteId} while execution site is ${sid}`);}
    return [...new Set(out)];
  };
  ...
  ProtoLab.reconcileAllForecastsV1100 = state => {let changed=0;for(const r of state?.requests||[])if(ProtoLab.reconcileRequestForecastV1100(state,r))changed++;return changed;};
  ProtoLab.networkMetricsV1099 = state => {
    ProtoLab.ensureMultiLabModelV1099(state);const rows=state.networkTransfers||[],accepted=rows.filter(x=>x.status==='Accepted'),sister=accepted.filter(x=>x.fromSiteId!==x.toSiteId);
    return {sisterLabBuilds:sister.length,recoveryTransfers:sister.filter(x=>(x.transferMode||'recovery')==='recovery').length,electiveTransfers:sister.filter(x=>x.transferMode==='elective').length,internallyRetainedHours:accepted.reduce((n,x)=>n+Number(x.plannedHours||0),0),outsourcingAvoided:accepted.reduce((n,x)=>n+Number(x.avoidedExternalCost||0),0),externalSpend:rows.filter(x=>x.status==='External').reduce((n,x)=>n+Number(x.externalCost||0),0)};
  };
