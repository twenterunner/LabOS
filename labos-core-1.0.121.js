(function(){
  ...
  ProtoLab.networkMetricsV1099 = state => {
    ...
    return {sisterLabBuilds:rows.filter(x=>x.status==='Accepted'&&x.scope!=='task'&&x.fromSiteId!==x.toSiteId).length,sisterLabOperations:taskRows.length,sisterLabTests:taskRows.filter(x=>String(x.taskKind||'').toLowerCase()==='test').length,sisterLabProcessSteps:taskRows.filter(x=>String(x.taskKind||'').toLowerCase()==='process').length,internallyRetainedHours:rows.filter(x=>x.status==='Accepted').reduce((n,x)=>n+Number(x.plannedHours||0),0),outsourcingAvoided:rows.filter(x=>x.status==='Accepted').reduce((n,x)=>n+Number(x.avoidedExternalCost||0),0),externalSpend:rows.filter(x=>x.status==='External').reduce((n,x)=>n+Number(x.externalCost||0),0)};
  };
