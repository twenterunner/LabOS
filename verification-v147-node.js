...
ok('Version is REV 1.0.47 / schema 22',()=>P.VERSION==='1.0.47-poc'&&P.SCHEMA_VERSION===22);
...
ok('User manual and shell identify REV 1.0.47',()=>/REV 1.0.47/.test(index)&&/REV 1.0.47/.test(manual)&&/styles\.css\?v=1.0.47/.test(index));
...
ok('Readiness blocker opens contextual resolver',()=>/function readinessBlockerModal/.test(app)&&/Only the failed booking\(s\) are shown/.test(app)&&/Fix now/.test(app)&&/Readiness blocker resolved/.test(app));
ok('Equipment resolver shows capability, assigned equipment and calibration/maintenance evidence',()=>/has capability/.test(app)&&/Calibration is not valid through the planned use date/.test(app)&&/Maintenance is not valid through the planned use date/.test(app)&&/Use \$\{x\.alts\[0\]\.name\}/.test(app)&&/Schedule \$\{x\.care\.type\.toLowerCase\(\)\}/.test(app));
...
ok('Process data has explicit setup + sample matrix flow',()=>/DATA FOR THIS PROCESS STEP/.test(app)&&/BATCH \/ SETUP/.test(app)&&/SAMPLE MATRIX/.test(app));
...
ok('Every sample field offers same-value bulk fill',()=>/Same value for all samples/.test(app)&&/data-pd-fill-all/.test(app)&&/Applied to \$\{samples\.length\} sample/.test(app));
...
ok('Accept starts implementation rather than silently closing action',()=>/Accept & implement/.test(app)&&/implementAccepted/.test(app)&&/improvementImplementationModal/.test(app));
ok('Improvement has four-step implementation rail',()=>/function improvementGuidedSteps/.test(app)&&/Receive & register equipment/.test(app)&&/commission/i.test(app)&&/Verify effectiveness/.test(app));
...
// REV 1.0.47 audit, configuration, resource-assurance and UX checks
ok('Audit scope is mandatory and empty scope is Major',()=>/Audit scope is defined/.test(app)&&/scopeOk\?'Ready':'Major'/.test(app)&&/Organisation, site\/lab and audit scope statement are required/.test(app));
ok('Audit evidence pack is blocked until scope is defined',()=>/Define the organisation, site\/laboratory and audit scope before generating the audit pack/.test(app)&&/auditProfileModal\(\);return/.test(app));
ok('Audit trail Minor has a guided retention-reference resolution',()=>/recordsRetentionReference/.test(app)&&/area==='audit trail'/.test(app)&&/Add controlled reference/.test(app));
ok('ISO internal-audit Minor resolves through controlled audit reference',()=>/internalAuditReference/.test(app)&&/Internal-audit programme \/ record is referenced/.test(app)&&/area==='internal audit'/.test(app));
ok('Every open audit finding exposes guided Resolve action',()=>/data-audit-resolve=/.test(app)&&/1 · Resolve/.test(app)&&/2 · Evidence/.test(app)&&/3 · Recheck/.test(app)&&/auditFindingAffected/.test(app));
ok('Audit evidence index includes key controlled record sets',()=>['Calibration register & certificate index','Maintenance register & evidence index','Competence / training matrix & certificate index','Controlled methods / work-instruction register','Sample genealogy / traceability records','Quality / nonconformity / corrective-action register','Change & approval audit trail'].every(x=>app.includes(x)));
ok('Customer master is restored under configuration',()=>/CUSTOMERS & REQUIREMENTS/.test(app)&&/data-add-customer/.test(app)&&/data-edit-customer/.test(app));
ok('Request list supports customer filter and requested sort modes',()=>/id="reqCustomer"/.test(app)&&/id="reqSort"/.test(app)&&/required-desc/.test(app)&&/sortBy==='product'/.test(app)&&/sortBy==='customer'/.test(app)&&/sortBy==='priority'/.test(app));
ok('Calibration and maintenance durations are configurable',()=>/Calibration duration \(hours\)/.test(app)&&/Maintenance duration \(hours\)/.test(app)&&/calibrationDurationHours/.test(app)&&/maintenanceDurationHours/.test(app));
ok('Training renewal duration is configurable in competency master',()=>/Renewal training duration \(hours\)/.test(app)&&/trainingDurationHours/.test(app));
ok('Calibration maintenance and training can be directly scheduled',()=>/data-new-care="Calibration"/.test(app)&&/data-new-care="Maintenance"/.test(app)&&/data-new-care="Training"/.test(app)&&/function careNewScheduleModal/.test(app)&&/svc\.scheduleAt/.test(app));
ok('Readiness schedules can be printed together or per activity',()=>/data-care-print="All"/.test(app)&&/data-care-print="Calibration"/.test(app)&&/data-care-print="Maintenance"/.test(app)&&/data-care-print="Training"/.test(app));
ok('Calibration maintenance and training reports expose evidence links',()=>/Open certificate/.test(app)&&/Open maintenance record/.test(app)&&/evidenceDocumentLink/.test(app));
ok('Primary Quality and workspace navigation no longer owns PFMEA',()=>!/PFMEA &amp; risks/.test(app)&&/case'processrisk':return workspaceFlow/.test(app)&&/tabs=\[\['actions','Action board'/.test(app));
ok('Current user documentation does not present PFMEA as a LabOS workflow',()=>!/PFMEA/i.test(manual));
ok('Progressive disclosure styling exists for compact secondary guidance',()=>/details\.fold\.compact/.test(css)&&/audit-guided-row/.test(css));
...
(async()=>{
 ...
 fs.writeFileSync(path.join(root,'VERIFICATION_RESULTS_v1.0.47.json'),JSON.stringify({version:P.VERSION,passed:pass,failed:fail,total:pass+fail,rows},null,2));
