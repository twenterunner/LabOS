...
/* ============================================================
   LabOS REV 1.0.163 — shared Programme model.
...
/* ============================================================
   LabOS REV 1.0.163 — enterprise KPI definition + event model.
...
(function(P){
  ...
  P.seedKpiDemoFactsV151=state=>{
    ...
    state.settings.kpiDemoSeedV151='1.0.163';state.settings.kpiModelVersion='1.0.163';return {changed:true,history:hist.length,delays:state.kpiFactsV151.delays.length,quality:state.kpiFactsV151.quality.length,capacity:state.kpiFactsV151.capacity.length};
  };
})(window.ProtoLab);

/* ============================================================
   LabOS REV 1.0.163 — unified visual Programme Builder model.
...
(function installProgrammeBuilderV162(P){
  ...
  function migrate(state,type,id){
    ...
    const structure={id:key(type,id),programmeType:type,entityId:id,modelVersion:'1.0.163',createdAt:now(),updatedAt:now(),layoutDirection:'LR',overlay:'structure',lanes:clone(src.lanes),activities:clone(src.activities),templatesUsed:[],revisionHistory:[]};
  ...
  P.ensureProgrammeBuilderV162=(state,{type=null,id=null}={})=>{
    ...
    state.settings=state.settings||{};if(!state.settings.programmeBuilderV162){state.settings.programmeBuilderV162={modelVersion:'1.0.163',migratedAt:now()};changed=true}
    return {changed};
  };
...
/* ============================================================
   LabOS REV 1.0.163 — integrated Validation planning + automatic
...
(function installV163IntegratedPlanningAndLearning(P){
  ...
  function captureValidation(state,p){
    ...
    const acts=(state.validationActivities||[]).filter(x=>x.programmeId===p.id),res=(state.validationResults||[]).filter(x=>x.programmeId===p.id),reqs=(state.validationRequirements||[]).filter(x=>x.programmeId===p.id),bookings=(state.bookings||[]).filter(x=>x.requestId===p.id&&!P.isHistoricalPlanningBooking?.(x)),fails=res.filter(x=>x.outcome==='Fail'),devs=acts.filter(x=>x.kind==='Test Development'),overruns=acts.filter(a=>Number(a.actualHours)>Math.max(1.25*Number(a.estimatedHours||0),Number(a.estimatedHours||0)+1));
